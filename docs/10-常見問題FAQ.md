...
檢查 log 輸出。常見原因：
1. `import/{year}/Q{quarter}/` 目錄中沒有 `.xlsx` 檔案（例如 `import/115/Q1/`）
2. `config/application.yml` 中的 `process-year` 未正確設定
3. 檔名格式不符規定
4. 來源資料有誤導致中止
...
只要在 `import/{year}/Q{quarter}/` 目錄放入該公司的來源 `.xlsx` 檔案即可（例如 `import/115/Q1/`）。系統會自動從來源資料中讀取公司資訊，程式化產生報表時會自動包含所有有資料的公司。
